...
//Raymarching 
float map( vec3 p );
float box( vec3 p, vec3 data )
{
    return max(max(abs(p.x)-data.x,abs(p.y)-data.y),abs(p.z)-data.z);
}
vec4 raymarche( vec3 org, vec3 dir, vec2 nfplane )
{
...
//Shading
vec3 shade( vec4 p, vec3 n, vec3 org, vec3 dir )
{		
	//wall color
	vec3 amb = vec3(.5);
	if(p.x<-9.999)
...
vec4 effect( vec4 color, Image texture, vec2 texture_coords, vec2 screen_coords ){
	...
	//camera ray with mouse
	//vec3 org = vec3(0.0, 10.0, -15.0);
	//vec3 dir = normalize( vec3(uv.x, uv.y, 1.5) + vec3(mouse,0.0));
	//dir = lookat( -org + vec3(0., 5., 0.), vec3(0., 1., 0.) ) * dir;
